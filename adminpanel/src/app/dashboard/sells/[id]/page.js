...
export default function SellDetailPage() {
  ...
  useEffect(() => {
    async function fetchSell() {
      try {
        const res = await apiFetch(`/api/admin/sells/${id}`);
        if (res.ok) setSell((await res.json()).sell);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  const handleAction = async (action) => {
    ...
    try {
      ...
      if (res.ok) {
        toast.success(`Sell request ${action === 'approve' ? 'approved' : 'rejected'} successfully`);
        setActionModal(null);
        setRemark('');
        const updated = await apiFetch(`/api/admin/sells/${id}`);
        if (updated.ok) setSell((await updated.json()).sell);
      } else {
  ...
  if (!sell) return <div className="text-center text-gray-500 mt-10">Sell request not found</div>;
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-gray-900">Sell Request #{sell.id}</h2>
          <StatusBadge status={sell.status} />
        </div>

        <div className="space-y-0">
          <InfoRow label="Amount" value={`$${sell.amount.toFixed(2)}`} />
          <InfoRow label="Reference ID" value={sell.referenceId} />
          <InfoRow label="Payment Method" value={sell.paymentMethod || 'IMPS'} />
          <InfoRow label="User" value={
            <Link href={`/dashboard/users/${sell.user?.id}`} className="text-blue-600 hover:underline">
              {sell.user?.phone || '—'}
            </Link>
          } />
          <InfoRow label="Phone" value={sell.user?.phone} />
          <InfoRow label="Payee Name" value={sell.payeeName} />
          <InfoRow label="Account No" value={sell.accountNo} />
          <InfoRow label="IFSC" value={sell.ifsc} />
          <InfoRow label="Bank Name" value={sell.bankName} />
          <InfoRow label="Admin Remark" value={sell.adminRemark} />
          <InfoRow label="Created" value={new Date(sell.createdAt).toLocaleString()} />
          {sell.reviewedAt && <InfoRow label="Reviewed" value={new Date(sell.reviewedAt).toLocaleString()} />}
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full shadow-2xl">
            <h3 className="text-lg font-bold text-gray-900 mb-4">
              {actionModal === 'approve' ? 'Approve' : 'Reject'} Sell #{sell.id}
            </h3>
