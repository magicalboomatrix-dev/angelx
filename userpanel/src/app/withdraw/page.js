...
export default function WithdrawUSDT() {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        // Store detail for the detail page
        const withdrawalData = {
          id: data.withdrawal?.id,
          type: 'WITHDRAW',
          amount: parseFloat(amount),
          status: 'PENDING',
          currency: data.withdrawal?.currency || selectedWallet.currency || 'USDT',
          network: selectedWallet.network || 'TRC20',
          referenceId: data.withdrawal?.referenceId || '',
          createdAt: new Date().toISOString(),
          reviewedAt: null,
          address: selectedWallet.address || '',
          adminRemark: null,
        };
        sessionStorage.setItem("withdraw_detail", JSON.stringify(withdrawalData));
        setAmount("");
        fetchBalance();
        router.push(`/withdraw-detail?id=${data.withdrawal?.id}`);
      } else {
