...
export default function WithdrawalDetailPage() {
  ...
  useEffect(() => {
    async function fetchWithdrawal() {
      try {
        ...
        if (res.ok) setWithdrawal((await res.json()).withdrawal);
      } catch (err) {
        console.error(err);
      } finally {
        setLoading(false);
  ...
  const handleAction = async (action) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setActionModal(null);
        setRemark('');
        const updated = await apiFetch(`/api/admin/withdrawals/${id}`);
        if (updated.ok) setWithdrawal((await updated.json()).withdrawal);
      } else {
  ...
  if (!withdrawal) return <div className="text-center text-gray-500 mt-10">Withdrawal not found</div>;
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="bg-white rounded-xl border border-gray-100 p-6 shadow-sm">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-gray-900">Withdrawal #{withdrawal.id}</h2>
          <StatusBadge status={withdrawal.status} />
        </div>

        <div className="space-y-0">
          <InfoRow label="Amount" value={`${withdrawal.amount.toFixed(2)} ${withdrawal.currency}`} />
          <InfoRow label="Network" value={withdrawal.network} />
          <InfoRow label="Reference ID" value={withdrawal.referenceId} />
          <InfoRow label="Wallet Address" value={withdrawal.walletAddress} />
          <InfoRow label="User" value={
            <Link href={`/dashboard/users/${withdrawal.user?.id}`} className="text-blue-600 hover:underline">
              {withdrawal.user?.phone || '—'}
            </Link>
          } />
          <InfoRow label="Phone" value={withdrawal.user?.phone} />
          <InfoRow label="Admin Remark" value={withdrawal.adminRemark} />
          <InfoRow label="Created" value={new Date(withdrawal.createdAt).toLocaleString()} />
          {withdrawal.reviewedAt && <InfoRow label="Reviewed" value={new Date(withdrawal.reviewedAt).toLocaleString()} />}
        </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full shadow-2xl">
            <h3 className="text-lg font-bold text-gray-900 mb-4">
              {actionModal === 'approve' ? 'Approve' : 'Reject'} Withdrawal #{withdrawal.id}
            </h3>
