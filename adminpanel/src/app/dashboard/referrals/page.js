    ...
    <div className="flex flex-col gap-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div className="flex flex-col gap-1.5">
          ...
          <p className="max-w-3xl text-sm text-slate-500 sm:text-[0.95rem]">Track reward payouts and monitor referral relationships across the full user graph.</p>
        </div>
      ...
      <section className="relative overflow-hidden  border border-white/15 bg-[radial-gradient(circle_at_15%_20%,rgba(255,255,255,0.18),transparent_24%),linear-gradient(135deg,#0f172a_0%,#182642_42%,#125b72_100%)] p-6 text-white shadow-[0_28px_60px_rgba(15,23,42,0.28)] lg:p-8">
        <div className="relative z-10 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <h2 className="mt-3 text-3xl font-semibold">{referrals.length} referral rewards recorded</h2>
            <p className="mt-3 max-w-xl text-sm text-slate-200/85">Each approved reward stays visible here with both users and payout totals for quick reconciliation.</p>
          </div>
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <div className=" border border-white/10 bg-white/8 px-4 py-3 backdrop-blur-sm">
              <p className="text-xs uppercase tracking-[0.24em] text-slate-300">Referral count</p>
              <p className="mt-2 text-2xl font-semibold">{referrals.length}</p>
            </div>
          </div>
        </div>
      </section>
      ...
      <div className="overflow-hidden  border border-slate-200/70 bg-white/85 shadow-[0_16px_34px_rgba(15,23,42,0.08)] backdrop-blur-xl">
        <div className="overflow-x-auto">
          <table className="min-w-[720px] w-full border-separate border-spacing-0 text-sm">
            ...
            <tbody>
              {loading ? (
                <tr><td colSpan={5} className="px-4 py-12 text-center text-sm text-slate-500">Loading referrals...</td></tr>
              ) : referrals.length === 0 ? (
                <tr><td colSpan={5} className="px-4 py-12 text-center text-sm text-slate-500">No referrals found</td></tr>
              ) : (
                referrals.map((r) => (
                  <tr key={r.id} className="transition hover:bg-slate-50/80">
                    ...
                    <td className="border-b border-slate-200/80 px-4 py-4 text-right font-semibold text-emerald-600">${r.rewardAmount.toFixed(2)}</td>
