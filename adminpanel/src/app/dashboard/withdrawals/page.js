...
export default function WithdrawalsPage() {
  ...
  const fetchWithdrawals = useCallback(async (p = 1) => {
    ...
    try {
      ...
      const res = await apiFetch(`/api/admin/withdrawals?${params}`);
      if (res.ok) {
  ...
  const handleAction = async (id, action) => {
    ...
    try {
      const res = await apiFetch(`/api/admin/withdrawals/${id}/${action}`, {
        method: 'PUT',
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={9} className="text-center py-8 text-gray-500">No withdrawals found</td></tr>
              ) : (
                withdrawals.map((w) => (
                  <tr key={w.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition cursor-pointer"
                    onClick={() => router.push(`/dashboard/withdrawals/${w.id}`)}>
                    <td className="px-4 py-3 text-gray-500">#{w.id}</td>
                    <td className="px-4 py-3">
                      <div className="font-medium">{w.user?.phone || '—'}</div>
                      <div className="text-xs text-gray-400">{w.user?.phone || ''}</div>
                    </td>
                    <td className="px-4 py-3 text-right font-semibold">{w.amount.toFixed(2)}</td>
                    <td className="px-4 py-3">{w.currency} ({w.network})</td>
                    <td className="px-4 py-3 text-xs text-gray-500 max-w-[120px] truncate" title={w.walletAddress}>{w.walletAddress || '—'}</td>
                    <td className="px-4 py-3"><StatusBadge status={w.status} /></td>
                    <td className="px-4 py-3 text-xs text-gray-500 max-w-[120px] truncate" title={w.adminRemark}>{w.adminRemark || '—'}</td>
                    <td className="px-4 py-3 text-gray-500 text-xs">{new Date(w.createdAt).toLocaleString()}</td>
                    <td className="px-4 py-3 text-center" onClick={(e) => e.stopPropagation()}>
                      {w.status === 'PENDING' && (
                        <div className="flex gap-1 justify-center">
                          <button onClick={() => setActionModal({ id: w.id, action: 'approve' })} className="p-1.5 rounded-lg bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition" title="Approve"><CheckCircle size={16} /></button>
                          <button onClick={() => setActionModal({ id: w.id, action: 'reject' })} className="p-1.5 rounded-lg bg-red-50 text-red-600 hover:bg-red-100 transition" title="Reject"><XCircle size={16} /></button>
                        </div>
                      )}
                    </td>
                  </tr>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-between px-4 py-3 border-t border-gray-100">
            ...
            <div className="flex gap-2">
              <button onClick={() => fetchWithdrawals(page - 1)} disabled={page <= 1} className="px-3 py-1.5 text-sm border rounded-lg disabled:opacity-40 hover:bg-gray-50">Previous</button>
              <button onClick={() => fetchWithdrawals(page + 1)} disabled={page >= totalPages} className="px-3 py-1.5 text-sm border rounded-lg disabled:opacity-40 hover:bg-gray-50">Next</button>
            </div>
          </div>
        )}
      </div>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl p-6 max-w-md w-full shadow-2xl">
            <h3 className="text-lg font-bold text-gray-900 mb-4">
              {actionModal.action === 'approve' ? 'Approve' : 'Reject'} Withdrawal #{actionModal.id}
            </h3>
