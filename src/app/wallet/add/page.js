...
const handleSubmit = async () => {
  ...
  try {
    const response = await fetch('/api/crypto-wallets', {
      ...
      body: JSON.stringify({ address, network: 'TRC20' }),
    });
      ...
      <div className={styles.formContent}>
        <div className={styles.formRow}>
          ...
          <div className={styles.networkDisplay}>
            <img src="/images/tb-ic1.png" alt="TRC20" className={styles.networkLogo} />
            TRC20–USDT
          </div>
        </div>

        <div className={styles.formRow} style={{alignItems: 'flex-start'}}>
          <span className={styles.label}>Wallet<br />address</span>
          <textarea
        ...
        <div className={styles.actionContainer}>
          <button
            className={address ? `${styles.commitBtn} ${styles.commitBtnEnabled}` : styles.commitBtn}
        ...
        <div className={styles.warningBox}>
          <p className={styles.warningText}>
            Please check the information carefully before submission. If transfer issues occur due to incorrect information provided by user, it is the user's own responsibility.
          </p>
          {message && (
